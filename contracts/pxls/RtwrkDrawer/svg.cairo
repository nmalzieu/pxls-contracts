...
const DEFAULT_COLOR_COMPONENT = 242;
...
func get_color_from_pixel_color{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    grid_array: felt*
) -> (color: Color) {
    let pixel_color = cast(grid_array, PixelColor*)[0];
    if (pixel_color.set == TRUE) {
        return (color=pixel_color.color);
    } else {
        return (
            color=Color(red=DEFAULT_COLOR_COMPONENT, green=DEFAULT_COLOR_COMPONENT, blue=DEFAULT_COLOR_COMPONENT),
        );
    }
}
...
func append_svg_rects_from_pixel_grid{
    ...
    let (color: Color) = get_color_from_pixel_color(grid_array);
    ...
    return append_svg_rects_from_pixel_grid(
        grid_size,
        grid_array_len - PixelColor.SIZE,
        grid_array + PixelColor.SIZE,
        pixel_index + 1,
        new_destination_len,
        destination,
    );
